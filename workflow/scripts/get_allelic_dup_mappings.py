...
def get_dup(dup_bed_df , tig_name ,coord_start , coord_end, is_seg = False):
...
def process_segment(seg ):
    ...
    ref_samp, ref_hap = [x for x in manifest["sample"] if x in seg.reference_name][0] , [x for x in manifest["hap"] if x in seg.reference_name][0] 
    ...
    for i, row in nested_dups.iterrows():
        q_samp, q_hap = [x for x in manifest["sample"] if x in seg.reference_name][0] , [x for x in manifest["hap"] if x in seg.reference_name][0] 
